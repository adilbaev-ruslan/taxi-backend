        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <form method="POST" action="{{route('owner.roles.update', ['role' => $role->id])}}">
                        @method('PUT')
                        @csrf
                        <div class="mb-4">
                            <label class="text-xl text-gray-600">Name <span class="text-red-500">*</span></label></br>
                            <input type="text" class="border-2 border-gray-300 p-2 w-full" name="name" id="name" value="{{ $role->name }}">
                ...
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <div class="mb-4">
                        <div class="flex p-1">
                            ...
                            <form method="POST" action="{{ route('owner.roles.permissions.revoke', ['role' => $role->id, 'permission' => $role_permission->id]) }}" onsubmit="return confirm('Are you sure you want to delete this item?')">
                            @method('DELETE')
                            @csrf
                             <button type="submit" class="border border-red-500 bg-red-500 text-white rounded-md px-4 py-2 m-2 transition duration-500 ease select-none hover:bg-red-600 focus:outline-none focus:shadow-outline">{{ $role_permission->name }}</button>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mt-5">
                <div class="p-6 bg-white border-b border-gray-200">
                    <form method="POST" action="{{route('owner.roles.permissions', ['role' => $role->id])}}">
                        @csrf
                        <div class="mb-4">
                            <label class="text-xl text-gray-600">Permissions <span class="text-red-500">*</span></label></br>
                            <select name="permission" class="border-2 border-gray-300 p-2 w-full" id="permission">
